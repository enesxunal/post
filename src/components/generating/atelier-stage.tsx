...
import { Paintbrush, Palette, Sparkles, Wand2, X, ZoomIn } from "lucide-react";
      ...
      <div className="relative mx-auto aspect-square w-full max-w-[520px]">
        ...
        <div className="absolute left-1/2 top-1/2 w-[58%] -translate-x-1/2 -translate-y-1/2">
          ...
          >
            <div className="relative aspect-square overflow-hidden rounded-[22px] bg-gradient-to-br from-emerald-950 to-[#071a12]">
              {imageUrl ? (
                <button
                  ...
                  aria-label={`${dayName} görselini büyüt`}
                >
                  ...
                  <span className="absolute bottom-2 right-2 flex items-center gap-1 rounded-full bg-black/50 px-2.5 py-1 text-[10px] font-medium text-white opacity-0 backdrop-blur-sm transition group-hover:opacity-100">
            ...
            {!imageUrl ? (
              <motion.div className="absolute bottom-3 left-3 right-3 h-1 overflow-hidden rounded-full bg-white/10">
                <motion.div
        ...
        {workers.map(({ Icon, label, x, y, delay }) => (
          <motion.div
            ...
            style={{ left: x, top: y }}
            animate={{ y: isReady ? 0 : [0, -6, 0], opacity: isReady ? 0.35 : 1 }}
            transition={{
              duration: 1.8,
              repeat: isReady ? 0 : Number.POSITIVE_INFINITY,
              delay,
              ease: "easeInOut",
            }}
          >
            <motion.div
              ...
              animate={{ rotate: isReady ? 0 : [-4, 4, -4] }}
              transition={{
                duration: 2.2,
                repeat: isReady ? 0 : Number.POSITIVE_INFINITY,
                delay: delay + 0.2,
      ...
      <AnimatePresence>
        {expanded && imageUrl ? (
          ...
          >
            <motion.button
              type="button"
              className="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white backdrop-blur-sm"
              onClick={() => setExpanded(false)}
              aria-label="Kapat"
            ...
            >
              ...
              <img
                src={imageUrl}
                alt={dayName}
                className="max-h-[70vh] max-w-[70vw] rounded-[24px] object-contain shadow-[0_30px_80px_rgba(0,0,0,0.55)]"
              />
              <p className="mt-3 text-center text-sm text-white/70">{dayName}</p>
            </motion.div>
          </motion.div>
        ) : null}
      </AnimatePresence>
